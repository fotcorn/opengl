...
Object::Object(std::vector<GLfloat> points, std::vector<GLfloat> colors) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * points.size(), &points[0], GL_STATIC_DRAW); // copy data to GPU memory
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * colors.size(), &colors[0], GL_STATIC_DRAW); // copy data to GPU memory
